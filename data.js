...
const GAME_DATA = [
    ...
    // --- 案件一：网球王子 (4x4 标准题 - 9线索) ---
    // 逻辑核心：无 If-Then，依靠二元关联和排他绑定锁定
    {
        title: "乾汁中毒事件",
        difficulty: "进阶版",
        ...
        clues: [
            ...
            "手冢国光(S2)使用的是蓝色球拍(W2)。",
            // 6. 二元关联
            "拿着绷带(W3)的人正在更衣室(L3)里。",
            // 7. 排他绑定
            "乾贞治(S3)手里拿的不是红色球拍(W1)。",
            // 8. 排他绑定
            "待在社办(L2)里的人不是不二周助(S4)。",
            // 9. OR (二选一)
            "特制乾汁(W4)要么在不二周助(S4)手里，要么就在社办(L2)里（只能是其中一种情况）。"
        ],
        solution: "UzMtVzQtTDI=" // S3-W4-L2 (乾贞治-乾汁-社办)
    },

    // --- 案件二：喜羊羊与灰太狼 (4x4 标准题 - 9线索) ---
    // 逻辑核心：利用排他性解决 OR 带来的歧义
    {
        title: "青青草原陷阱之谜",
        difficulty: "进阶版",
        ...
        clues: [
            ...
            "平底锅(W1)没有出现在小河边(L3)。",
            "喜羊羊(S1)就在实验室(L4)里。",
            "拿着隐形药水(W3)的人正是喜羊羊(S1)。",
            "捕兽夹(W4)被安放在了羊村学校(L2)。",
            "灰太狼(S3)并没有拿捕兽夹(W4)。",
            "在小河边(L3)的羊并没有拿平底锅(W1)。",
            "慢羊羊(S4)要么在小河边(L3)，要么在羊村学校(L2)。"
        ],
        solution: "UzMtVzEtTDE=" // S3-W1-L1 (灰太狼-平底锅-狼堡)
    },

    // --- 案件三：名侦探柯南 (5x5 Boss战 - 14线索) ---
    // 逻辑核心：必须使用“反证法”（假设S2在L3会导致逻辑崩塌）
    {
        title: "米花大饭店暗杀事件",
        difficulty: "专家级",
        ...
        clues: [
            ...
            // OR/XOR x2
            "琴酒(S5)要么在饭店天台(L3)，要么小兰(S2)在饭店天台(L3)。（二者必居其一）",
            "狙击枪(W5)要么在米花公园(L4)，要么空手道(W2)在米花公园(L4)。",
            
            // If-Then (反证核心)
            "关键线索：如果小兰(S2)在饭店天台(L3)，那么狙击枪(W5)也一定在饭店天台(L3)。",
            
            // 弱肯定 (收敛)
            "柯南(S1)正在侦探所(L1)里思考案情。"
        ],
        solution: "UzUtVzUtTDM=" // S5-W5-L3 (琴酒-狙击枪-天台)
    }
];